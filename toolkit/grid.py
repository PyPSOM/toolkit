...
def psom_grid_seamount(Lx, Ly, Max_depth, Min_depth, res, ktx_param, kty_param):
    ...
    kty = {'amplitude':1,'center':0.8*Ly,'std':0.02*Ly,'shelf_extention':0,'z':y}
    ktx = {'amplitude':1,'center':0.5*Lx,'std':0.02*Lx,'shelf_extention':0,'z':x}

    kty1 = {'amplitude':1,'center':0.3*Ly,'std':0.02*Ly,'shelf_extention':0,'z':y}
    ktx1 = {'amplitude':1,'center':0.5*Lx,'std':0.02*Lx,'shelf_extention':0,'z':x}

    f = (g.subs(kty)*g.subs(ktx)*dz  + g.subs(kty1)*g.subs(ktx1)*dz) -(dz-Min_depth)
